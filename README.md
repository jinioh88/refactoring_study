...
- 절차
  - 대입문의 우변에서 부작용이 생기지는 않는지 확인
  - 변수가 불변으로 선언되지 않앗다면 불변으로 만든 후 테스트
    - 변수에 값이 단 한 번만 대입되는지 확인할 수 있다. 
  - 이 변수를 가장 처름 사용하는 코드를 찾아 대입문 우변의 코드로 바꾸고 테스트
  - 변수를 사용하는 부분을 모두 교체할 때까지 이 과정 반복
  - 변수 선언문과 대입문을 지운다.
  - 테스트한다. 
...
- 절차
  - 절차가 두 가지가 잇다. 
  - 리팩터링할 때 함수 선언과 호출문들을 단번에 고칠 수 잇다면 간단한 절차를 따른다. 
  - 복잡하면 마이그레이션 절차를 적용해 호출문들을 점진적으로 수정한다. 
  - 간단한 절차
    - 매개변수를 제거하려거든 먼저 함수 본문에서 제거 대상 매개변수를 참조하는 곳은 없는지 확인한다. 
    - 메서드 선언을 원하는 형태로 바꾼다. 
    - 기존 메서드 선언을 참조하는 부분을 모두 찾아 바뀐 형태로 수정하고 테스트한다. 
    - 변경할 게 둘 이상이라면 예를들어 이름 변경과 매개변수 추가를 모두 하고 싶다면 각각을 독립적으로 처리하자. 
  - 마이그레이션 절차
    ...
    - 테스트 한다. 
    - 기존 함수를 인라인 한다. 
    - 이름을 임시로 붙여뒀다면 함수 선언 바꾸기를 한 번 더 적용해 원래 이름으로 되돌린다.
    - 테스트한다. 
  - 다형성을 구현한 클래스 즉, 상속 구조 속에 있는 클래스의 메서드를 변경할 땐 다형 관계인 다른 클래스들에도 변경이 반영되야 한다. 
    - 단일 상속 구조라면 전달 메서드를 슈퍼클래스에 정의하면 해결된다. 
  - 공개된 API를 리팩토링할 때는 새 함수를 추가한 다음 리팩터링을 잠시 멈출 수 있다. 
    - 이 상태에서 예전 함수를 폐기 대상으로 지정하고 모든 클라이언트가 새 함수로 이전할 때가지 기다린 후, 클라이언트들이 모두 이전했다는 확신이 들면 예전 함수를 지운다. 
- 예시: 함수 이름 바꾸기(간단한 절차)
  ```javascript
